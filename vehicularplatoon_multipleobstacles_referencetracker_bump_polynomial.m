...
obstacle_pos = [  153  120  235 ;
                  150   80  242 ];
r_obs        = [  20   10   30 ];
n_obs        = size(obstacle_pos,2);

v_lead  = [16; 16];          
r       = 2.5*v_lead .* t;       % nominal straight-line reference



%% States: [x1;y1;phi1; x2;y2;phi2; x3;y3;phi3]
X       = zeros(9, N);
X(:,1)  = [-100; -100; 0;   0; 0; 0;   0; 0; 0];  % initial poses
...
for k = 1:N-1
    ...
    pt2  = p2 + (l+15)*[cos(phi2); sin(phi2)];
    pt3  = p3 + (l+30)*[cos(phi3); sin(phi3)];
    

   
    u_dash = [(r(:,k) - pt1);      % leader tracks modified reference
              (pt1 - pt2);         % follower 1 tracks leader
              (pt2 - pt3) ];       % follower 2 tracks follower 1
