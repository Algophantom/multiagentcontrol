...
r = [100; 100];
 ...
 for i=1:N-1
    ...
    theta = omega * t(i);       % rotation angle at time t(i)

    % 2D rotation matrix
    R = [cos(theta), -sin(theta); sin(theta), cos(theta)];

    % Triangle formation in body frame
    offset1 = [0; 2];               % Top vertex
    offset2 = [-sqrt(3); -1];       % Bottom left vertex
    offset3 = [sqrt(3); -1];        % Bottom right vertex

    % Rotate and translate to get desired positions
    p1d = r + R * offset1;   % Desired position of Drone 1
    p2d = r + R * offset2;   % Desired position of Drone 2
    p3d = r + R * offset3;   % Desired position of Drone 3
