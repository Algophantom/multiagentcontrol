...
for i = 1:N-1
   ...
   u(3:6,i+1) = u(3:6,i+1) + c_avoid_2*[avoid2; avoid3];%updated velocities with avoid states
    ...
    X(3:4,i+1) = X(3:4,i) + dt*u(3:4,i+1);%final position for the instant delayed by 2 instants
    X(5:6,i+1) = X(5:6,i) + dt*u(5:6,i+1);%final position for the instant delayed by 6 instants
end
...
function [avoid_vel, avoid_state] = multi_obsavoidance(pos, obs_pos, r_obs, avoid_state, k, m, dt)
    ...
    for j = 1:n_obs
        ...
        if d < (r_obs(j) + safety)%include a safety barrier
            ...
            damping = 0.2;
            avoid_state= (damping)*avoid_state + dt * (k/m) * overlap;%modelled as spring damper system(NI)
