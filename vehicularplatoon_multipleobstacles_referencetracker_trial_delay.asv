...
v_lead = [40; 40];%velocityoftheleader
...
for i = 4:N-1
    ...
    udot        = 9*u_dash - 20*u(:,i);%consensus NI controller
    %controller(s) = 1/(s+1)
    u(:,i+1)  = u(:,i) + dt*udot;%adding the change to the followers 

    pos1 = X(1:2,i);
    pos2 = X(3:4,i);
    pos3 = X(5:6,i);

    [avoid1, avoid_state1,avoid_on1] = multi_obsavoidance(pos1, obstacle_pos, r_obs, avoid_state1, k, m, dt);%avoidance functions
    [avoid2,  avoid_state2, avoid_on2] = multi_obsavoidance(pos2, obstacle_pos, r_obs, avoid_state2, k, m, dt);
    [avoid3,  avoid_state3,avoid_on3] = multi_obsavoidance(pos3, obstacle_pos, r_obs, avoid_state3, k, m, dt);
    
    
    
    %e_ref = r(:,i) - X(1:2,i);
    %if avoid_on1
     %   u(1:2,i+1) = v_des + 4*e_ref + c_avoid*avoid1;
    %else
     %   u(1:2,i+1) = v_des + c_avoid*avoid1;
    %end
    %alpha = 0.4*double(~avoid_on1); 
    u(1:2,i+1) = u(1:2,i+1) +  c_avoid_2*avoid1; % Update leader's velocity based on avoidance
    ...
    X(5:6,i+1) = X(5:6,i) + dt*u(5:6,i-3);%final position for the instant
end
...
function [avoid_vel, avoid_state,avoid_active] = multi_obsavoidance(pos, obs_pos, r_obs, avoid_state, k, m, dt)
    ...
    for j = 1:n_obs
        ...
        if d < (r_obs(j) + safety)%include a safety barrier
            ...
            avoid_state= dt * (k/m) * overlap;%modelled as spring system(NI)
